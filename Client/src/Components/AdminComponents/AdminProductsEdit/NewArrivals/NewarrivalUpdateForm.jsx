...
const NewarrivalUpdateForm = () => {
  ...
  const removeSizeField = (indexToRemove) => {
    if (formData.sizes.length === 1) return; // Prevent removing last item
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`https://api.photoparkk.com/api/newarrivals/${id}`, data, {
        headers: {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          {formData.sizes.map((size, index) => (
            <div
              key={index}
              className="grid grid-cols-3 sm:grid-cols-4 gap-2 mb-2 items-center"
            >
              <input
                type="text"
